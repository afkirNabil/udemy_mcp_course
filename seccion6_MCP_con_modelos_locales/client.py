...
class GmailMCPClient:
    def __init__(self):
        # Ahora en vez de instanciar un Cliente de OpenAi lo hacemos de Ollama. Y definimos el modelo.
        self.ollama_model = os.getenv("OLLAMA_MODEL", "qwen3:1.7b")
        # Invocamos el servicio de ollama, q se encuentra corriendo en el puerto 11434 de localhost
        self.ollama_host = os.getenv("OLLAMA_HOST", "http://localhost:11434")

        # client = ollama.Client(host=self.ollama_host)
        # client.chat()
        # Inicializar el cliente asi no hace falta pq ya lo hace la libreria automaticamente. Es decir lee del .env la varible de entorno directamente.
    ...
    async def get_tools_for_llm(self):
        """Convierte herramientas del Servidor MCP al formato de OpenAI/LLM para mandar esas herramientas al modelo LLM."""
        async with await self._get_mcp_client() as cliente:
    ...
    async def chat(self, messages: list) -> str:
        ...
        async with await self._get_mcp_client() as mcp_client:
            ...
            # LLamada inicial a OLLAMaA cliente inicializado en el init de la Clase.
