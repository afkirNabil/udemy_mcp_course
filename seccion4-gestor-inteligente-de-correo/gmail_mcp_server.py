...
def get_gmail_service():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file('credentials-gmail-scopes.json', SCOPES)
...
# ============================================== RESOURCES ==============================================
# Recurso que proporciona información sobre el perfil de Gmail
# Aunq tenga acciones, metodos o funciones. No modifica nada, es de solo consulta. Constula con sencillas, nada complejo. Si fuese un consulta SQL, mejor hacerla con Herramientas

@mcp.resource("gmail://profile") #URI
def get_profile() -> str:
    """
    Recursos: Información del perfil del usuario de Gmail
    """
    service = get_gmail_service()
    profile = service.users().getProfile(userId='me').execute()
    output = "# Perfil de Gmail\n\n"
    output += f"**Email:** {profile['emailAddress']}\n"
    output += f"**Total de mensajes:** {profile['messagesTotal']}\n"
    output += f"**Total de hilos:** {profile['threadsTotal']}\n"
    return output

# ============================================== RESOURCES TEMPLATES ==============================================
# Lo mismo que resources pero con variables dinamicas en la URI.
# Para mostrar informacion de manera dinamica.

@mcp.resource("docs://setup-manual/{version}")
def get_setup_manual(version: str = "latest") -> str:
    """
    Resouce Template: Manual de configuración del gestor de correos desde archivos PDF
    ...
    # Mapeo de versiones a archivos PDF
    version_map = {
    ...
    # Determinar el archivo PDF correcto
    pdf_filename = version_map.get(version.lower())

    if not pdf_filename:
        return f"Error: La versión '{version}' no ha sido encontrada."
    
    # Construir la ruta completa al archivo PDF
    manuals_dir = os.path.join(os.path.dirname(__file__), "manuals") # uno la ruta de este fichero con la carpeta manuals
    pdf_path = os.path.join(manuals_dir, pdf_filename)

    # Verificar si el archivo PDF existe
    if not os.path.exists(pdf_path):
        return f"Error: El archivo '{pdf_filename}' no existe en la carpeta de manuales '{pdf_path}'"
    ...
    try:
        with open(pdf_path, "rb") as pdf_file:
            ...
            output = f"# Manual de Configuración - Versión {version.upper()}\n\n"
            output += f"**Archivo:** {pdf_filename}\n"
            output += f"**Número de páginas:** {num_pages}\n"
